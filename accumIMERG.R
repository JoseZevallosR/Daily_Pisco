...
AccumGPM<-function( 
  ...
  init<-substr(basename(tt[1]),34,37)
    ...
    if (paralelled==T){ 
      ...
      if (SO=='Winbug'){ 
        ...
        foreach(i=1:(length(tt)%/%48))%dopar%{ 
          ...
          substr(basename(tt[((i-1)*48+1):(i*48)])[1],24,31),".tif"),overwrite=T)}
          stopCluster(cl)}
    if(paralelled==F) for(i in 1:(length(tt)%/%6)) writeRaster(sum(stack(tt[((i-1)*48+1):(i*48)]),na.rm = T),
          paste0("IMERG.Daily.",substr(basename(tt[((i-1)*48+1):(i*48)])[(i-1)*48+1],24,31),".tif"))
  }        
